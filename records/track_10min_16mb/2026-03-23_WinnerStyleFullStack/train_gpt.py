...
class Hyperparameters:
    ...
    skip_gated_unet = bool(int(os.environ.get("SKIP_GATED_UNET", "1")))
    ...
    ttt_mlp_only = bool(int(os.environ.get("TTT_MLP_ONLY", "1")))
    ttt_cosine_decay = bool(int(os.environ.get("TTT_COSINE_DECAY", "0")))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,skip_gates,smear,dtg_gate,ve_layer_scales,ve_shared.scale",
    ).split(",")
    if pattern
)
...
class CastedLinear(nn.Linear):
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        if self.qat_active and self.training and w.ndim == 2:
            with torch.no_grad():
...
def set_qat_enabled(module: nn.Module, enabled: bool) -> int:
    count = 0
    for submodule in module.modules():
        if isinstance(submodule, CastedLinear):
            submodule.qat_active = enabled
            count += 1
    return count
...
class GPT(nn.Module):
    def __init__(
        ...
        self.skip_gated_unet = skip_gated_unet
        self.skip_weights = nn.Parameter(torch.ones(self.num_skip_weights, model_dim, dtype=torch.float32))
        self.skip_gates = (
            nn.Parameter(torch.full((self.num_skip_weights, model_dim), 2.0, dtype=torch.float32))
            if skip_gated_unet and self.num_skip_weights > 0
            else None
        )
...
def ttt_adapt(
    ...
    if args.ttt_mlp_only:
        for name, p in base_model.named_parameters():
            if ".mlp." in name or "ve_shared.proj" in name:
                p.requires_grad_(True)
    elif args.ttt_freeze_blocks > 0:
        for i, block in enumerate(base_model.blocks):
            requires_grad = i >= args.ttt_freeze_blocks
            for p in block.parameters():
                p.requires_grad_(requires_grad)
        if hasattr(base_model, "skip_weights"):
            base_model.skip_weights.requires_grad_(True)
        if getattr(base_model, "skip_gates", None) is not None:
            base_model.skip_gates.requires_grad_(True)
        if getattr(base_model, "ve_layer_scales", None) is not None:
    ...
    if not ttt_params:
        if log_fn:
            log_fn("ttt:skipped no_trainable_params")
        for p in base_model.parameters():
            p.requires_grad_(True)
        return
    ...
    scheduler = None
    if args.ttt_cosine_decay and args.ttt_epochs > 1:
    ...
    for epoch in range(args.ttt_epochs):
        ...
        if scheduler is not None:
            scheduler.step()
        if log_fn:
            avg_loss = epoch_loss_sum.item() / max(epoch_tokens.item(), 1.0)
            log_fn(f"ttt_epoch:{epoch+1}/{args.ttt_epochs} loss:{avg_loss:.4f} elapsed:{time.perf_counter()-t0:.1f}s")
...
def main() -> None:
    ...
    base_model = GPT(
        ...
        skip_gated_unet=args.skip_gated_unet,
    ...
    initial_qat_layers = set_qat_enabled(base_model, args.qat_enabled)
    def build_training_model() -> nn.Module:
    ...
    def qat_probe_delta() -> float:
        probe_linear = next((m for m in base_model.modules() if isinstance(m, CastedLinear) and m.weight.ndim == 2), None)
        if probe_linear is None:
            return 0.0
        probe_x = torch.randn(2, probe_linear.weight.shape[1], device=device, dtype=torch.bfloat16)
        w = probe_linear.weight.float()
        row_max = w.abs().amax(dim=1)
        scale = (row_max / 31.0).clamp_min(1.0 / 31.0)
        w_q = (torch.clamp(torch.round(w / scale[:, None]), -32, 31) * scale[:, None]).to(dtype=probe_x.dtype)
        with torch.no_grad():
            ref = F.linear(probe_x, probe_linear.weight.to(dtype=probe_x.dtype))
            qat = F.linear(probe_x, w_q)
        return float((qat - ref).abs().amax().item())

    model: nn.Module = build_training_model()

    matrix_params_enc, matrix_params_dec = [], []
    scalar_params_enc, scalar_params_dec = [], []
    for i, block in enumerate(base_model.blocks):
        is_decoder = i >= base_model.num_encoder_layers
    ...
    if base_model.skip_weights.numel() > 0:
        scalar_params_dec.append(base_model.skip_weights)
    if base_model.skip_gates is not None:
        scalar_params_dec.append(base_model.skip_gates)
    ...
    if base_model.ve_shared is not None:
        ...
        if base_model.ve_shared.proj is not None:
            other_matrix_params.append(base_model.ve_shared.proj.weight)
    ...
    if base_model.mtp_num_heads > 0:
        optimizer_muon.param_groups[1]["params"].extend([p for p in base_model.mtp_heads.parameters() if p.ndim == 2])
    ...
    log0(
        ...
        f"matrix_lr:{args.matrix_lr} scalar_lr:{args.scalar_lr} decoder_lr_mult:{args.decoder_lr_mult}"
    )
    log0(
    ...
    log0(
        f"rope_dims:{args.rope_dims} ln_scale:{args.ln_scale} skip_gated_unet:{args.skip_gated_unet} "
        f"ve_enabled:{args.ve_enabled} ttt_enabled:{args.ttt_enabled}"
    )
    log0(f"qat_enabled:{args.qat_enabled} qat_layers:{initial_qat_layers}")
    ...
    while True:
        ...
        qat_is_active = any(
            isinstance(submodule, CastedLinear) and submodule.qat_active
            for submodule in base_model.modules()
        )
        if args.late_qat_threshold > 0 and scale < args.late_qat_threshold and not qat_is_active:
            qat_layers = set_qat_enabled(base_model, True)
            if distributed:
                dist.barrier()
            ...
            log0(
                ...
                f"recompiled:1 qat_layers:{qat_layers} probe_delta:{qat_probe_delta():.6e}"
            )
        zero_grad_all()
    ...
    if args.ttt_enabled:
        if distributed:
            dist.barrier()
        log0(
            f"ttt:start lr:{args.ttt_lr} epochs:{args.ttt_epochs} "
            f"freeze_blocks:{args.ttt_freeze_blocks} mlp_only:{args.ttt_mlp_only}"
        )
        ttt_adapt(
            args,
            base_model,
    ...
    eval_model = GPT(
        ...
        rope_dims=args.rope_dims, ln_scale=args.ln_scale, skip_gated_unet=args.skip_gated_unet, dtg=args.dtg_enabled,
